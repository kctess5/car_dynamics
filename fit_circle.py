...
def center(x_data, y_data):
    return (mean(x_data), mean(y_data))

def fit_leastsq(x_data, y_data):
    # method_2  = "leastsq"
    def calc_R(xc, yc):
        """ calculate the distance of each 2D points from the center (xc, yc) """
        return sqrt((x_data-xc)**2 + (y_data-yc)**2)
    ...
    center_2, ier = optimize.leastsq(f_2, center(x_data, y_data))
    ...
    return {
        "center": (xc_2, yc_2),
        "radius": R_2,
        "num_calls": f_2.ncalls
    }

def fit_leastsq_jacobian(x_data, y_data):
    ...
    def Df_2b(c):
        ...
        xc, yc     = c
        df2b_dc    = empty((len(c), x_data.size))

        Ri = calc_R(xc, yc)
        df2b_dc[ 0] = (xc - x_data)/Ri                   # dR/dxc
        df2b_dc[ 1] = (yc - y_data)/Ri                   # dR/dyc
        df2b_dc       = df2b_dc - df2b_dc.mean(axis=1)[:, newaxis]

        return df2b_dc

    center_2b, ier = optimize.leastsq(f_2b, center(x_data, y_data), Dfun=Df_2b, col_deriv=True)
    xc_2b, yc_2b = center_2b
    Ri_2b        = calc_R(xc_2b, yc_2b)
    R_2b         = Ri_2b.mean()

    return {
        "center": [float(xc_2b), float(yc_2b)],
        "radius": R_2b,
        "num_calls": f_2b.ncalls
    }
